    ...
    setup() {
      ...
      const columns = [
        ...
        },
        {
          ...
          sorter: (a: TableDataType, b: TableDataType) => Date.parse(a.check_at) - Date.parse(b.check_at),
          onFilter: (value: string, record: TableDataType) =>
      ...
      const handleModify = (key: string) => {
        ...
        for (let i in itemData.value) {
          if (itemData.value[i].key === key) {
            ...
            isChecked.value = itemData.value[i].teacher_check_at ? true : false
          }
        }
      ...
      const handleModifyOk = () => {
        ...
        teacherOnDutyCon.value = checkDay
        proxy.$api.get(
            '/updEvenCheckItem',
            {},
            {
